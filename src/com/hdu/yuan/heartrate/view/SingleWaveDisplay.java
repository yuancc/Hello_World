...
public class SingleWaveDisplay extends SurfaceView implements SurfaceHolder.Callback  {
	private DisplayThread displayThread;
	
	int numPtsToDraw=256;
//	int numPtsToDraw=400;
	...
	public SingleWaveDisplay(Context context,AttributeSet attributeSet) {
	...
	public void Draw(Canvas canvas){
		...
		for(int i=0;i<numPtsToDraw-1;i++){
			double y_i=(_range-DataToDraw[i])/(_range)*height;
			double y_i1=(_range-DataToDraw[i+1])/(_range)*height;
			//Log.d(TAG, "y_i="+y_i+" DataToDraw[2*i]="+DataToDraw[i]);
...
//		}
	...
	private void drawCoordinate(Canvas canvas,Paint p){
		//canvas.drawLine(-width/2.0f  , 0, width/2.0f-1.0f , 0,  coordinate_paint);//画水平线
		canvas.drawLine(0, -height/2.0f , 0, height/2.0f , coordinate_paint);//画纵坐标
		for(int horizontal = 1; horizontal<10; horizontal++){
		    ...
		    canvas.drawLine(
		    			...
		    			10.0f, -height/2.0f + 2*horizontal*(height/20),
		    			coordinate_paint);	
		    canvas.drawText(numberformat.format((10 - horizontal)*_range/5.0f),
	...
	private class DisplayThread extends Thread{
		...
		private SingleWaveDisplay _SingleWave;
		private boolean runningFlag = false;
		private long startTime;
		private long endTime;
		private long realendTime;
		
		public DisplayThread(SurfaceHolder surfaceHolder,SingleWaveDisplay singleWave) {//���췽��
